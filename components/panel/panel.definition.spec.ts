...
describe('Panel Contract Tests', function () {
  it('should ensure the contract is clear', function () {
    ...
    expect(schema.properties).toEqual({
      ...
      accordionCollapsed: { type: 'boolean' },
      modal: { type: 'boolean' },
      outsideDismiss: { type: 'boolean' },
      triggerOnDismissal: { type: 'string' },
      modalOpenEvent: { type: 'string' },
      modalDismissEvent: { type: 'string' },
      windowClass: { type: 'string' },
      isolateValidation: { type: 'boolean' },
      childIds: { type: 'array', description: 'Child ids for nestable references' },
      styling: {
        $ref: '#/definitions/PanelStyling',
      },
    })
  })
})
